...
export const createPages: GatsbyNode['createPages'] = async ({ graphql, actions }) => {
    ...
    return graphql<any>(`
        query GatsbyNodeQuery {
            ...
            halloffame: allMdx(filter: { fields: { sourceName: { eq: "halloffame" } } }) {
                edges {
                    node {
                        id
                    }
    ...
    `).then(result => {
        ...
        const blogPosts = result.data.halloffame.edges
        ...
        Array.from({ length: numBlogPages }).forEach((_, i) => {
            createPage({
                path: i === 0 ? `/halloffame` : `/halloffame/${i + 1}`,
                component: path.resolve("./src/templates/halloffame-list.tsx"),
                context: {
